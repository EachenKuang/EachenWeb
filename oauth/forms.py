...
class BindEmail(forms.Form):
    ...
    email = forms.EmailField(label='注册邮箱',
                             widget=forms.EmailInput(
                                 attrs={'class': 'form-control', 'id': 'email', 'placeholder': '请输入您注册用的邮箱'}))
    pwd = forms.CharField(label='用户密码', max_length=36,
                          widget=forms.PasswordInput(
                              attrs={'class': 'form-control', 'id': 'pwd', 'placeholder': '若尚未注册过，该密码则作为用户密码'}))
    ...
    def clean_email(self):
        ...
        if users:
            ...
            if OAuth_ex.objects.filter(user=users[0], oauth_type=oauth_type):
                raise ValidationError('该邮箱已经被绑定了')
        return email
    ...
    def clean_pwd(self):
        ...
        if users:
            ...
            if user is not None:
                ...
                return ValidationError('密码不正确，不能绑定')